...
export default class Part8ObjectAndClassAndOOP {
  constructor() {
    ...
    // 8.1.1 属性的类型
    // 1. 数据属性
    // 这些属性是为 JS 实现引擎的规范而定义的，因此，开发者不能直接访问
    // 两个方括号括起来的四个属性
    ...
    try {
      ({ name: personName, foo: { bar: personBar }, age: personAge} = personG);
    } catch (error) {
    ...
    // 1 在内存中创建了一个对象
    // 将新对象的[[prototype]]，指向当前构造函数的 proptype 原型
    // 构造函数内部的 this 指向新对象
    // 执行构造函数内的代码（将属性赋值到对象上）
    // 如果构造函数返回非空对象，则返回对象，否则返回这个新创建的对象
    ...
    // 在实例化时，如果没有参数，后面的括号可以不写，不过个人觉得还是统一一下
    // 这里用函数表达式的方式来创建构造函数
    ...
    const personWithObj = new PersonWithObj;
    console.info(personWithObj.name); // new P
    console.info(personWithObj instanceof PersonWithObj); // false

    // 类构造函数与构造函数主要区别是，调用类构造函数必须使用 new  操作符
    // 而普通构造函数如果不适用 new 调用，会使用全局的 this（一般是 window) 来作为内部对象
    ...
    // 派生类会通过原型链访问类和原型上定义的方法 this 的值会反映调用相应方法的实例或者类
    class Vehicle {
    ...
    // * super() 的行为给如同调用构造函数，如果需要给构造函数传参，则需要手动传入
    class Veh5 {
      constructor(license) {
    ...
    // 目前没有专门支持这种类的预发，但是在构造函数中使用 new.target 可以检测
    // 基类
    class VehicleBasic {
      constructor() {
    ...
    const bus1 = new BusBasic(); // 正常执行
    const veh1 = new VehicleBasic(); // 抛出异常
    ...
    const superArray1 = new SuperArray(1, 3, 4, 5, 6);
    const superArray2 = new SuperArray(1, 3, 4, 5, 6);
