...
// 使用 bind 和 call 实现函数，可以遍历类数组对象
const mapForArrayLike = Function.prototype.call(Array.prototype.map);
mapForArrayLike(document.querySelectorAll('*'), console.info);
...
function debounce(fn, timeout) {
  let timer;
  let ags = [].slice(arguments, 2);
  return function (...args) {
    if (timer) {
      clearTimeout(timer);
    }
    timer = setTimeout(() => {
      fn.apply(this, args.concat(ags));
    }, timeout);
  }
}
...
function throttle(fn, timeout) {
  ...
  return function t(...args) {
    const a = args.concat(ags);
    const current = Date.now();
    console.info('first Time', current, startTime);
