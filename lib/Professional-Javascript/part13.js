...
export default class Part13EVENT {
  constructor() {
    ...
    // 13.3 事件对象
    // 13.1.1 DOM 中的事件对象
    let mySecondBtn = document.querySelector('#mySecondBtn');
    mySecondBtn.onclick = function(event) {
      console.info(event, event.type); // click
      console.info(event.target === this);
      console.info(event.currentTarget === this);
    };

    mySecondBtn.addEventListener('click', function (event) {
      console.info(event, event.type); // click
    });
    ...
    document.body.onclick = function(event) {
      ...
      console.info(event.target === mySecondBtn); // true 当前调用对象
    };
