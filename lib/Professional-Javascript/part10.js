...
export default class Part10DOM {
  constructor() {
    ...
    console.info(body.lastChild, commentDom); // 不是最后一个

    // 10.1.8 DocumentFragment 类型
    // 文档片段
    let fragment = document.createDocumentFragment();
    let mySecondList = document.getElementById('mySecondList');
    ...
    mySecondList.appendChild(fragment);

    // 10.1.9 Attr 类型
    let attr = document.createAttribute('align');
    attr.value = 'left';
    mySecondList.setAttributeNode(attr);
    console.info(mySecondList.getAttributeNode('align').value); // left
    console.info(mySecondList.getAttribute('align')); // left
