...
export default class Part11DOMP {
  constructor() {
    ...
    // 返回 DOMTokenList
    this.handleClassList(getBody(null, "body"), "body", "add");
    console.info(getBody().classList); // ['body']

    this.handleClassList(getBody(null, "body"), "body", "contains"); // true

    this.handleClassList(getBody(null, "body"), "body", "remove");
    console.info(getBody().classList); // ['']
    // toggle 如果有，就删除，如果没有，就添加，以一代三
    this.handleClassList(getBody(null, "body"), "newBody", "toggle");
    console.info(getBody().classList); // ['newBody']
