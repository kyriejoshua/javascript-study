...
export default class Part23Storage {
  constructor() {
    ...
    // 批量读取，采用数组作为入参的方式
    transactions = db.transaction(['users', 'anotherStore'])

    const IDBTransaction = this.getIDBTransaction()

    // 指定第二个参数，来标识是只读还是读写
    let writableTransaction = db.transaction('users', IDBTransaction.READ_WRITE)
    ...
    let adminRequest = indexDB.open('admin'),
      database
    ...
    let setVersionRequest = database.setVersion('2.0')
