...
const classModule = () => {
   ...
   */
  class RangeNativeInES6Child extends RangeNativeInES6 {
...
/**
 * 创建不可变对象 Immutable Obj
 * @param {Object} obj
 */
 function deepFreeze(obj) {
  let propsName = obj.getOwnPropertyNames();
  Array.isArray(propsName) && propsName.forEach((name) => {
    let prop = obj[name]
    if (typeof prop === 'object' && prop !== null) { // null 也是 object 对象
      // 递归调用冻结对象方法
      deepFreeze(prop);
    }
  })
...
// 利用 Promise 特性创建时序函数
[func1, func2, func3].reduce((p, f) => { return p.then(f) }, Promise.resolve())
// Promise.resolve() 会返回一个 promise 对象，因此使用 reduce 方法可以让数组内的方法不断调用下去
// 注意 reduce 方法内的函数一定要 return，继续返回这个 promise 对象
// 上述方法等同于
